...
//    }
    ...
    public void admitHero(Knight Knight) {
        if (hasAlreadyHeroIn()) {
    ...
    public void kickHero(Knight Knight) {
        if (hasAlreadyHeroIn(Knight)) {
            System.out.println(this.hero.getName() + " has been removed from " + this.levelType.getName());
            this.hero = null;
            this.status = LevelStatus.NOT_STARTED;
            Knight.setCurrentLevel(null);
        } else {
            System.out.println("Sorry, " + Knight.getName() + " is not in this level.");
        }
    ...
    private boolean hasAlreadyHeroIn(Knight Knight) {
        return this.hero != null && this.hero.equals(Knight);
    }
